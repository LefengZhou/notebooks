...
* 引理：$\sigma$ 可延拓成 $\sigma': F(u)\hookrightarrow K'$ $\iff$ $f'(x)$ 在 $K'$ 有根
  * 证明：若有根 $u'$，根据两个同构域的扩域同构定理，有 $F(u)\cong F'(u')\hookrightarrow K'$ 。反过来，如果有这样的延拓，则 $f(u) = 0 \Longrightarrow \sigma'(f(u)) = 0 \Longrightarrow f'(\sigma'(u))$ 。可见 $\sigma'(u)$ 是根。
...
* 同构延拓定理：$E$ 是 $f(x)$ 在 $F$ 上的分裂域（分裂域的存在性保证）。$E'$ 是 $f'(x) = \sigma(f(x))$ 在 $F'$ 上的分裂域。$\sigma$ 可延拓成 $\sigma':E\tilde \rightarrow E'$ 。延拓个数 $m$ 满足 $1\leq m\leq [E:F]$ 。
  * 每添加一个根来单扩张，$f(x)$ 将会在扩域分裂成 $f(x)=g(x)h(x)$ ，其中 $g(x)$ 的根全部在扩域中（即在扩域完全分裂），$h(x)$ 在扩域不可约。根据引理，总延拓个数会乘上 $g(x)$ 相异的根的个数。扩张次数会乘上 $\deg g$ 。$g$ 相异根的个数小于等于 $g$ 的次数，当没有重根时取等。然后用单扩张来分裂 $h(x)$ ，不断单扩张直至得到分裂域。可知，$f$ 相异的根的个数小于等于 $f$ 的次数，当没有重根时取等，即 $f$ 可分时取等。

伽罗瓦群 $\mathrm{Gal}(F/K)$

* 定义：扩域上的自同构，在底域为恒等映射，对映射合成运算成群。$F/K,\sigma:F\tilde\rightarrow F,\sigma_K = \mathrm{Id}$ 。
...
* $|F| = p^n, n = [F:\mathbb{Z}_p]$
  * 证明思路：利用环的特征。存在 $\phi : \mathbb{Z}_p \hookrightarrow F, \bar n \mapsto n\cdot1_F$ 。有域扩张 $F/\mathbb{Z}_p$ 。$F$ 是 $\mathbb{Z}_p$ 上线性空间，维数为 $n$ 。
...
* $p^n$ 阶域唯一。
  * 根据定理二，$\forall F,|F| = p^n, \forall a\in F^*$ ，有 $a^{p^n-1} = 1$ 。所以 $a$ 是 $g(x)$ 的根，任意 $p^n$ 元域是 $g(x)$ 的根集。（注意此时还不能说明唯一性，因为元素之间的关系不清楚）同时有 $F = \mathbb{Z}_p(u)$ ，则 $F$ 是 $g(x)$ 在 $\mathbb{Z}_p$ 上的分裂域。分裂域在同构意义下唯一。
...
* 构造方法：
  ...
  * $F = \mathbb{Z}_p(u) \cong \mathbb{Z}[x]/(f(x)), a = a_0+a_1 u+a_2 u^2+\cdots +a_{n-1}u^{n-1} \in F$
